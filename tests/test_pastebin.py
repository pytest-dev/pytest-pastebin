...
class TestPasteCapture:
    ...
    def pastebinlist(
        ...
        plugin = request.config.pluginmanager.getplugin("pastebin2")
    ...
    def test_all(
        ...
        matcher = pytest.LineMatcher(contents.splitlines())
...
class TestPaste:
    @pytest.fixture
    def pastebin(self, request: pytest.FixtureRequest):
        return request.config.pluginmanager.getplugin("pastebin2")

    @pytest.fixture
    def mocked_urlopen_invalid(self, monkeypatch: pytest.MonkeyPatch):
    ...
    @pytest.fixture
    def mocked_urlopen(self, monkeypatch: pytest.MonkeyPatch):
        """Monkeypatch the actual urlopen calls done by the internal plugin
    ...
    def test_create_new_paste_failure(
        self,
        pastebin,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
