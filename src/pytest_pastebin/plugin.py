...
pastebinfile_key = pytest.StashKey[IO[bytes]]()
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    try:
        group.addoption(
            ...
            default=None,
            choices=["failed", "all"],
            help="Send failed|all info to bpa.st pastebin service",
        )
    except ValueError:
        # Pytest builtin pastebin added the equivalent option.
        pass


def pytest_load_initial_conftests(early_config: pytest.Config) -> None:
    early_config.pluginmanager.set_blocked("pastebin")
